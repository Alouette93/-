...
void PIT_IRQHandler(void)
{
    if(PIT_FLAG_GET(PIT_CH0))
    {
			  ...
			  if(uart_data=='a'&&InterfaceCfg.MenuStatus!=INFO_SUCCESS)
				{
					InterfaceCfg.MenuStatus=INFO_SUCCESS;
					duty=10000;
					fast_gpio_toggle(C15);
					systick_delay_ms(5);
					fast_gpio_toggle(C15);
				}
	      if(uart_data=='b'&&InterfaceCfg.MenuStatus!=INFO_WAITING)
				{
					InterfaceCfg.MenuStatus=INFO_WAITING;
					duty=5000;
				}
				/*if(uart_data==45)
				{
